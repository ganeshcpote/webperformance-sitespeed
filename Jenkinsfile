      ...
      steps {
        script{
          if ("${performance_tool}" == "sitespeedtest"){
            ...
            sh 'docker run --rm -v "$(pwd):/sitespeed.io" sitespeedio/sitespeed.io:17.1.1-plus1 ${site_url} --graphite.host=192.168.0.7  --slug ${test_name}  --plugins.add analysisstorer --plugins.add /lighthouse  -d ${crawl_depth} -m ${crawl_maxPages} --outputFolder output'
          }
	else if ("${performance_tool}" == "gpsi"){
            sh 'docker run --rm -v "$(pwd):/sitespeed.io" sitespeedio/sitespeed.io:17.1.1-plus1 ${site_url} --graphite.host=192.168.0.7  --slug ${test_name}  --plugins.add analysisstorer --plugins.add /gpsi  -d ${crawl_depth} -m ${crawl_maxPages} --outputFolder output'
          }
          else{
