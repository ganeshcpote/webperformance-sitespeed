      ...
      steps {
        script{
          if ("${performance_tool}" == "sitespeedtest"){
            ...
            sh 'docker run --rm -v "$(pwd):/sitespeed.io" sitespeedio/sitespeed.io:17.1.1 --influxdb.host=${influxdb_host} ${site_url} --influxdb.username=root --influxdb.password=root --influxdb.database=sitespeeddb  --slug ${test_name} -b ${browser}  -d ${crawl_depth} -m ${crawl_maxPages} --outputFolder output --budget.configPath budget-old.json --budget.output junit --budget.suppressExitCode true'		  
          }
	    ...
	    sh 'docker run --rm -v "$(pwd):/sitespeed.io" sitespeedio/sitespeed.io:17.1.1 ${site_url} --influxdb.host=${influxdb_host} --influxdb.username=root --influxdb.password=root --influxdb.database=sitespeeddb_curx --slug ${test_name} --crux.key AIzaSyCP3qXPW-ZMa_TjxfOogEgbRsVRAspo4_4 --crux.formFactor ALL --crux.collect ALL -d ${crawl_depth} -m ${crawl_maxPages} --outputFolder output --budget.configPath budget-old.json --budget.output junit --budget.suppressExitCode true'
          }
	    ...
	    sh 'docker run --shm-size=1g --rm -v "$(pwd):/sitespeed.io" sitespeedio/sitespeed.io:17.0.0-plus1 ${site_url} --influxdb.host=${influxdb_host} --influxdb.username=root --influxdb.password=root --influxdb.database=sitespeeddb_lighthouse --slug ${test_name}  --plugins.add analysisstorer --plugins.add /lighthouse -d ${crawl_depth} -m ${crawl_maxPages} --outputFolder output'
          }
	    ...
	    sh 'docker run --rm -v "$(pwd):/sitespeed.io" sitespeedio/sitespeed.io:17.0.0-plus1 ${site_url} --influxdb.host=${influxdb_host} --influxdb.username=root --influxdb.password=root --influxdb.database=sitespeeddb_gpsi --slug ${test_name}  --plugins.add analysisstorer --plugins.add /gpsi  -d ${crawl_depth} -m ${crawl_maxPages} --outputFolder output --budget.configPath budget-old.json --budget.output junit --budget.suppressExitCode true'
          }
          else{
